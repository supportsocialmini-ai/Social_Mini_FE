    ...
    <div className="flex min-h-screen bg-[#f4f7fe] font-sans selection:bg-indigo-100 relative">
      ...
      <aside className={`w-72 bg-gradient-to-b from-[#4318FF] to-[#707EAE] fixed h-full left-0 top-0 text-white z-[60] flex flex-col shadow-2xl transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <nav className="flex-1 px-6 space-y-2">
          ...
          <NavItem icon="users" label="Users" active={activeTab === 'users'} onClick={() => { setActiveTab('dashboard'); setIsSidebarOpen(false); }} />
      ...
      <main className="flex-1 md:ml-72 p-4 md:p-10 pt-32 min-h-screen relative overflow-y-auto w-full">
        ...
        {/* TOPBAR */}
        <header className="fixed top-4 md:top-6 right-4 md:right-10 left-4 md:left-[calc(18rem+2.5rem)] h-20 md:h-16 bg-white/70 backdrop-blur-xl border border-white/50 rounded-2xl flex flex-col md:flex-row items-center justify-between px-4 md:px-6 z-40 shadow-xl shadow-slate-200/40 gap-3 md:gap-0 py-2 md:py-0">
          <div className="flex items-center gap-3 w-full md:w-auto">
            <button 
              onClick={() => setIsSidebarOpen(true)}
              className="md:hidden p-2 text-slate-500 bg-[#f4f7fe] rounded-lg border border-slate-100"
            >
              <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16m-7 6h7" />
              </svg>
            </button>
            <div className="flex items-center gap-4 bg-[#f4f7fe] px-4 py-2 rounded-xl border border-slate-100 flex-1 md:flex-none">
              ...
              <input 
                ...
                className="bg-transparent border-none outline-none text-sm font-medium text-slate-600 placeholder:text-slate-400 w-full md:w-64"
              />
            </div>
          </div>
          ...
          <div className="flex items-center justify-between w-full md:w-auto md:gap-6">
            ...
            <div className="flex items-center gap-3 pl-6 border-l border-slate-100">
              ...
              <div className="w-10 h-10 rounded-xl overflow-hidden border-2 border-indigo-100 shadow-md flex-shrink-0">
        ...
        <div className="space-y-6 md:space-y-10">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <h1 className="text-2xl md:text-3xl font-black text-[#1B2559] tracking-tight text-center sm:text-left">Overview</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-8">
            ...
            <div className="col-span-12 md:col-span-8 space-y-6 md:space-y-8">
              ...
              <div className="bg-white/90 backdrop-blur-md p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-white relative overflow-hidden">
                ...
                <div className="h-60 md:h-72 flex items-end justify-between gap-2 md:gap-4 px-2 relative overflow-x-auto pb-4 sm:pb-0">
                  ...
                  <BarChart value={stats?.totalUsers || 0} maxValue={100} label="01" color="#4318FF" />
                  <BarChart value={stats?.activeUsers || 0} maxValue={100} label="02" color="#6AD2FF" />
              ...
              {/* MAPS SECTION - Simplified as a Card */}
            ...
            {/* RIGHT COLUMN: CALCULATION & ANALYTICAL */}
          ...
          {/* USER MANAGEMENT SECTION (Styled as a Large Card) */}
          <div className="bg-white/90 backdrop-blur-md rounded-[2rem] md:rounded-[3rem] shadow-2xl shadow-slate-200/50 border border-white overflow-hidden pb-8">
            <div className="px-6 md:px-10 py-8 md:py-10 flex flex-col md:flex-row justify-between items-center gap-6">
              ...
              <div className="bg-[#4318FF] text-white px-6 py-2.5 rounded-2xl shadow-xl shadow-indigo-100 flex items-center gap-2">
                 ...
                 <span className="text-[10px] font-black uppercase tracking-widest opacity-80">Total Roles Found</span>
              </div>
            </div>
                    ...
                    <tr key={userItem.userId} className="hover:bg-[#f4f7fe]/50 transition-all group">
                      ...
                      <td className="px-8 py-6 text-right">
                        <button 
                          ...
                          className={`text-[10px] font-black uppercase tracking-widest px-6 py-3 rounded-2xl transition-all hover:-translate-y-0.5 active:scale-95 shadow-md ${
                            userItem.isActive 
