...
const GroupDetail = () => {
  ...
  const isGroupCreator = !!(group && (group.createdBy || group.CreatedBy) === user?.userId);
  const isGroupAdmin = isGroupCreator || isSystemAdmin;
  // Creator nhóm và System Admin đều có thể xóa nhóm
  const canDeleteGroup = isGroupCreator || isSystemAdmin;
  ...
  const handleDeleteGroup = async () => {
    try {
      ...
      console.error(error);
      toast.error(error?.response?.data?.message || 'Không thể xóa nhóm. Vui lòng thử lại.');
  ...
  return (
    <div className="min-h-screen" style={{
                ...
                <>
                  ...
                  {canDeleteGroup ? (
                    <button
                      ...
                      className="px-6 py-2.5 bg-red-600 text-white font-bold rounded-2xl hover:bg-red-700 transition-all flex items-center gap-2 shadow-lg shadow-red-200 hover:scale-105 active:scale-95"
                    >
          ...
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md mx-4 overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex border-b border-slate-100 px-6 bg-slate-50/50">
              <button
                ...
                className={`flex-1 py-3 text-sm font-black border-b-2 transition-all duration-200 ${activeTab === 'friends'
              ...
              <button
                ...
                className={`flex-1 py-3 text-sm font-black border-b-2 transition-all duration-200 ${activeTab === 'topicUsers'
                    ? 'border-violet-600 text-violet-600'
                    : 'border-transparent text-slate-400 hover:text-slate-600'
                  }`}
              >
      ...
      {showDeleteGroupConfirm && (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => !deletingGroup && setShowDeleteGroupConfirm(false)}>
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md mx-4 overflow-hidden" onClick={e => e.stopPropagation()}>
            {/* Header */}
            <div className="px-6 py-5 bg-gradient-to-r from-red-600 to-rose-600 flex items-center gap-4">
              ...
              <div>
                <h2 className="text-xl font-black text-white">Xóa nhóm</h2>
                <p className="text-red-100 text-sm font-medium truncate max-w-xs">{group?.name}</p>
              </div>
            </div>
            ...
            <div className="px-6 py-6">
              <div className="bg-red-50 border border-red-200 rounded-2xl p-4 mb-5">
                <div className="flex gap-3">
                  ...
                  <div>
                    <p className="text-red-700 font-black text-sm mb-1">Bạn sẽ không thể hoàn tác tác vụ này!</p>
                    <p className="text-red-600 text-sm leading-relaxed">
                      Toàn bộ dữ liệu của nhóm sẽ bị xóa vĩnh viễn bao gồm: tất cả bài viết, tin nhắn nhóm, danh sách thành viên và cài đặt nhóm.
                    </p>
                  </div>
                </div>
              </div>
              <p className="text-slate-600 text-sm leading-relaxed">
                {isSystemAdmin && !isGroupCreator
                  ? <>Đây là hành động của <span className="font-bold text-red-600">Quản trị viên hệ thống</span>. Nhóm </>
                  : <>Bạn là người tạo nhóm này. Nhóm </>
                }
                <span className="font-bold text-slate-800">&ldquo;{group?.name}&rdquo;</span> sẽ bị xóa hoàn toàn.
              </p>
            </div>
            ...
            <div className="px-6 pb-6 flex gap-3">
              <button
                ...
                className="flex-1 py-3.5 rounded-2xl bg-slate-100 text-slate-600 font-black text-sm hover:bg-slate-200 transition-all active:scale-[0.98] disabled:opacity-50"
              >
                Hủy bỏ
              </button>
              <button
                ...
                disabled={deletingGroup}
                className="flex-1 py-3.5 rounded-2xl bg-red-600 text-white font-black text-sm hover:bg-red-700 transition-all active:scale-[0.98] shadow-lg shadow-red-200 disabled:opacity-70 flex items-center justify-center gap-2"
              >
                {deletingGroup ? (
                  <>
