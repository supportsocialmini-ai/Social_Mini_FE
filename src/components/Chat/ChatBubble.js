...
const ChatWindow = ({ chatUser, onClose, connection, getFullAvatarUrl, currentUser, onlineUsers }) => {
  ...
  const handleImageSelect = async (e) => {
    ...
    if (!file.type.startsWith('image/')) {
      alert('Chỉ cho phép gửi file ảnh!');
      return;
    }
    ...
    try {
      ...
      if (imageUrlInput) {
        if (isGroup) {
          await connection.invoke('SendGroupMessage', conversationId, '', imageUrlInput);
        } else {
          await connection.invoke('SendPrivateMessage', chatUser.userId, '', imageUrlInput);
        }
      } else {
    ...
    >
      ...
      {!isMinimized && (
        <>
              ...
              return (
                <div key={i} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                  ...
                  <div className={`max-w-[85%] px-3 py-2 rounded-2xl text-[12px] font-medium shadow-sm ${
                    ...
                    {imgUrl ? (
                      <img src={imgUrl} alt="sent" className="max-w-full rounded-xl cursor-zoom-in hover:opacity-90 transition-opacity" onClick={() => setEnlargedImage(imgUrl)} />
                    ) : (
          ...
          {/* Input */}
          <form onSubmit={sendMessage} className="flex items-center gap-2 px-2.5 py-2.5 bg-white border-t border-slate-100">
            <input 
              type="file" 
              ref={imageInputRef} 
              className="hidden" 
            ...
            <button 
              ...
              className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all flex-shrink-0"
              title="Gửi ảnh"
            >
              {isUploading ? (
      ...
      {enlargedImage && (
        <div className="fixed inset-0 z-[25000] bg-black/90 flex items-center justify-center p-4 cursor-zoom-out animate-in fade-in duration-200" onClick={() => setEnlargedImage(null)}>
          <img src={enlargedImage} alt="" className="max-w-full max-h-[90vh] object-contain rounded-2xl animate-in zoom-in-95 duration-200" />
        </div>
      )}
    </div>
  );
